...
export function MergeTwoSortedListsVisualizer({ step, transitionDuration = 0.4 }: VisualizationProps) {
  const { visualizationData, variables } = step;
  const { linkedLists, stack, annotations } = visualizationData;
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          {linkedLists.map((list) => (
            <div key={list.id} className="flex flex-col items-center gap-4">
              <motion.h3
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              <div
                className="p-6 rounded-lg w-full flex justify-center"
                style={{
                  backgroundColor: theme.colors.surface,
        ...
        {stack && stack.length > 0 && (
          <div>
            <StackAdapter stack={stack} transitionDuration={transitionDuration} config={{ title: 'Call Stack' }} />
          </div>
        )}
      </div>
